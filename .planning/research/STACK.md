...
**Supabase tables needed (custom analytics):**
...
**Why built-in sitemap.ts over next-sitemap:**
...
```bash
# Core framework
npm install next@^14.2.25 react@^18 react-dom@^18

# Content layer
npm install velite zod

# MDX plugins
npm install @mdx-js/react rehype-pretty-code shiki rehype-slug rehype-autolink-headings remark-gfm
...
Velite uses Zod schemas to define content types. Define an `articles` collection with fields for title, description, category, pillar (boolean), publishDate, keywords, faqSchema, and computed fields for slug, readingTime, and wordCount.

### Tailwind v3 setup
Standard `tailwind.config.ts` with content paths, @tailwindcss/typography plugin, and tailwindcss-animate. Extend theme for brand colors (to be determined), font families, and custom animation keyframes.

### GDPR considerations
Since the site uses NO third-party cookies or tracking scripts (custom Supabase analytics uses server-side inserts via API routes, not client-side cookies), a complex cookie consent banner is unnecessary. A simple privacy notice in the footer linking to the privacy policy is sufficient. The email capture form needs explicit consent checkbox (not pre-checked) and link to privacy policy.

### Supabase analytics approach
Analytics are collected via Next.js API routes (POST requests from client) that insert into Supabase tables. No client-side cookies needed. The `user_agent` should be hashed server-side for privacy. Country detection via Vercel's `x-vercel-ip-country` header (free, no extra service needed).
